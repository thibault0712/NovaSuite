    ...
    <div>
      ...
      <div className='flex border-t border-b border-t-slate-500/30 border-b-slate-500/30 bg-slate-700'>
        <div className="flex text-slate-400 text-xs leading-6 overflow-x-auto scrollbar-none">
          {RenderDocumentsButtons(documents, selectedDocument, setSelectedDocument, open, setOpen, setUpdate, formData, setFormData)}
        </div>
        <div className='flex items-end ml-auto'>
          <button onClick={() => HandleNewDocument(documents, setDocuments, selectedDocument, setSelectedDocument, setUpdate)} className="flex-none text-gray-400 font-bold border-l border-r border-l-slate-500/30 border-r-slate-500/30 px-3 py-1 flex items-center">+</button>
          <Link className="h-full" to={'/'}>
            <button className="flex-none text-gray-400 font-bold border-l border-r border-l-slate-500/30 border-r-slate-500/30 px-3 py-1 flex items-center h-full"><MdRemoveRedEye /></button>
          </Link>
        </div>
      </div>

      <div className="overflow-x-auto">
        {RenderNodes(nodes, objectives, documents, selectedDocument, setUpdate, blockedNodes, formData, setFormData, setFile, file)}         
      </div>
    </div>
  );
}
