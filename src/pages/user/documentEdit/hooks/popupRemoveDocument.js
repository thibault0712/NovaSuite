...
export function PopupRemoveDocument(setOpen, selectedDocument, setUpdate, documents, setSelectedDocument){
    ...
    >
      {close => (
        ...
        <div className="modal bg-slate-700 rounded-xl pb-4 mx-4 my-6 shadow-md overflow-y-auto scrollbar-thin scrollbar-track-slate-700/0 scrollbar-thumb-slate-900/50" style={{ width: '85vw', maxWidth: '600px', maxHeight: '85vh' }}>
          <div className="mb-3 text-center text-xl py-3 font-bold text-white bg-slate-700 sticky top-0 z-10">Êtes-vous sûr de vouloir supprimer le document ?</div>
          ...
          <div className="pt-4 flex justify-end mr-5">
            <div style={{margin: '0 auto'}}>
                ...
                {documents.length > 1 && <button onClick={() => {HandleDeletDocument(selectedDocument, setUpdate, documents, setSelectedDocument); close()}} className="bg-red-500 w-30 text-white font-bold py-2 px-4 rounded">Confirmer</button>}
